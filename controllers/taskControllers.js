...
module.exports.fet = async (req, res, next) => {
  try {
    ...
    taskModel.find({userid}).then((tasks)=>{
...
module.exports.markt =async (req,res,next)=>{
    try{
        ...
        return res.status(200).json({msg:"Marked Done",status:true});
    }
    catch(ex){
        next(ex);
    }
...
module.exports.pending =async (req,res,next)=>{
    try{
        ...
        return res.status(200).json({msg:"List of all pending items",task,status:true});
    }
    catch(ex){
        next(ex);
    }
